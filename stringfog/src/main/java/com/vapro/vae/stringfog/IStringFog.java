 ...
 *
 * 设计要点：
 *   1. 字节级契约——只负责 byte[] ⇄ byte[] 变换，传输编码（Base64 文本 / 原始 bytes）由上层
 ...
 *   4. 线程安全契约——实现应为无状态或线程安全：同一实例会被构建期多线程插桩与运行时并发
 *      解密复用（StringFogRuntime 注册表跨线程共享同一实例），内置 XorFog/AesFog 均无可变字段。
 * ===============================================================================
 */
public interface IStringFog {
     ...
     * 加密：明文字节 → 密文字节（构建期调用）。
     *
     * @param data 明文字节（通常为字符串的 UTF-8 编码），非 null
     * @param key  密钥字节（算法内部自行归一化到所需长度），非 null
     ...
     * 解密：密文字节 → 明文字节（运行时调用）。
     *
     * @param data 密文字节（与 encrypt 输出一致，传输解码后传入），非 null
     * @param key  密钥字节，须与加密时完全一致，非 null
    ...
    /**
     * 判定某个字符串字面量是否值得加密。
     * <p>默认实现：过滤 null 与空串（避免常量池膨胀）；最小串长阈值由插件层
     * （{@code minLength}）叠加控制，与算法无关，故不在此判定。自定义算法可覆写以追加
     * 领域规则（例如跳过纯数字或已符合某格式的串）。</p>
     *
     * @param value 待判定的原始字符串字面量（可能为 null）
     * @return true 表示应加密，false 表示原样保留
     */
    default boolean shouldFog(String value) {
